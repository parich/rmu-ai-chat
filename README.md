...
เซิร์ฟเวอร์ WordPress resolve DNS ของโดเมน Dify ไม่ได้ — พบบนเซิร์ฟเวอร์ในเครือข่ายมหาวิทยาลัย (RMU)
ที่ DNS resolver ภายในปฏิเสธ query โดเมนภายนอก (`Query refused`)

**วิธีแก้ (กรณีเซิร์ฟเวอร์ Dify อยู่ในเครือข่ายเดียวกัน):** เพิ่ม entry ใน `/etc/hosts` ของเซิร์ฟเวอร์
WordPress ให้ชี้ตรงไปยัง IP ภายใน — traffic จะวิ่งใน LAN ตรง ไม่ต้องพึ่ง DNS ภายนอก และ TLS
ยังตรวจสอบ cert กับชื่อโดเมนได้ปกติ (Traefik ฝั่ง Coolify ถือ cert ของโดเมนอยู่แล้ว)

```bash
echo "202.29.22.52 dify.dokkoon.com" >> /etc/hosts
curl -I https://dify.dokkoon.com/v1/parameters   # ได้ HTTP status ใดๆ = ผ่าน
```

> ตัวอย่างจริง: cc.rmu.ac.th (202.29.22.64, CyberPanel) → Dify บน Coolify (202.29.22.52)
> แก้ผ่าน CyberPanel Web Terminal ได้โดยไม่ต้อง SSH — **ถ้าย้ายเซิร์ฟเวอร์ Dify/เปลี่ยน IP
> ต้องตามแก้บรรทัดนี้ทุกเครื่องที่เพิ่มไว้ด้วย**
...
```php
define( 'WP_ACCESSIBLE_HOSTS', 'api.wordpress.org,dify.dokkoon.com' );
```
